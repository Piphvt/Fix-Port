                ...
                <v-row justify="center" align="center">
                    <v-col cols="auto">
                        <div class="d-flex align-center justify-center">
                            ...
                            <v-autocomplete v-if="searchType === 'reach'" v-model="selectedReaches"
                                class="mx-2 search-size small-font" :items="getSearchItems('reach')"
                                label="ค้นหาสิ่งที่ถึงเป้า" dense outlined clearable multiple>
                            </v-autocomplete>
            ...
            <v-card-text>
                <v-data-table :headers="headers" :items="filteredFollows" item-value="no" item-key="no"
                    ...
                    <template v-slot:item.updated_date="{ item }">
                        <div class="text-center" style="color:#38b6ff">{{ formatDateTime(item.updated_date) }}</div>
                    </template>
                    <template v-slot:item.reach="{ item }">
                        <div class="text-center"
                            :style="{ color: item.reach === 1 ? '#ffc800' : item.reach === 2 ? '#00bf63' : '#000' }">
                            {{ getReachText(item.reach) }}
                        </div>
                    </template>
                            ...
                            <v-menu offset-y>
                                ...
                                <v-list class="custom-list">
                                    ...
                                    <v-list-item @click="showConfirmDialog('waiting', item)" class="custom-list-item">
                                        ...
                                        <v-list-item-content
                                            style="font-size: 0.8rem;">รอการตรวจสอบ</v-list-item-content>
                                    </v-list-item>
    ...
    data() {
        return {
            ...
            searchTypes: [
                ...
                { text: 'สิ่งที่ถึงเป้า', value: 'reach' },
            ],
            searchQuery: '',
            savedSearches: [],
    ...
    methods: {
        ...
        getSearchItems(type) {
            if (type === 'stock_no') {
                ...
                return this.follows
                    .filter(follow => this.getReachText(follow.reach))
                    .map(follow => this.getReachText(follow.reach));
            }
            return [];
        },
        ...
        addSearchItemsToSearch() {
            ...
            if (selectedItems.length > 0) {
                ...
                if (this.searchType === 'reach') {
                    this.selectedReaches = [];
                }
        ...
        applySearchFilter(follow, search) {
            ...
            } else if (search.type === 'reach') {
                field = this.getReachText(follow.reach);
            } else {
        ...
        exportExcel() {
            ...
            const worksheet = workbook.addWorksheet('สรุปหุ้นที่ถึงเป้าแล้ว');
            ...
            this.filteredFollows.forEach((item, index) => {
                ...
                this.headers.forEach(header => {
                    ...
                    } else if (header.value === 'updated_date') {
                        rowData[header.value] = moment(item[header.value]).tz('Asia/Bangkok').format('YYYY-MM-DD HH:mm');
                    ...
                    } else if (header.value === 'reach') {
                        rowData[header.value] = this.getReachText(item.reach) || 'ยังไม่ระบุ';
            ...
            workbook.xlsx.writeBuffer().then(buffer => {
                ...
                link.setAttribute('download', `สรุปหุ้นที่ถึงเป้าแล้ว-${currentDate}.xlsx`);
